...
import com.mpe.financial.struts.form.DepartmentForm;
...
public class DepartmentAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//DepartmentForm uomForm = (DepartmentForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		DepartmentForm form = (DepartmentForm) actionForm;
		...
		try {
			...
			Criteria criteria = DepartmentDAO.getInstance().getSession().createCriteria(Department.class);
			...
			// partial data
			criteria = DepartmentDAO.getInstance().getSession().createCriteria(Department.class);
			if (form.getString("code")!=null && form.getString("code").length()>0)criteria.add(Restrictions.like("Code", "%"+form.getString("code")+"%"));
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			...
			try {
				DepartmentDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			if (form.getLong("departmentId") == 0) {
				form.setString("departmentId",0);
				...
				Department obj = DepartmentDAO.getInstance().get(form.getLong("departmentId"));
				form.setString("departmentId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			Department obj = null;
			if (form.getLong("departmentId") == 0) {
				obj = (Department)DepartmentDAO.getInstance().getSession().createCriteria(Department.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new Department();
					...
					DepartmentDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj.setId(form.getLong("departmentId"));
				DepartmentDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			DepartmentDAO.getInstance().delete(form.getLong("departmentId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
