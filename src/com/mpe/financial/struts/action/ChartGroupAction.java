...
import com.mpe.financial.struts.form.ChartGroupForm;
...
public class ChartGroupAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//ChartGroupForm uomForm = (ChartGroupForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		ChartGroupForm form = (ChartGroupForm) actionForm;
		...
		try {
			...
			Criteria criteria = ChartGroupDAO.getInstance().getSession().createCriteria(ChartGroup.class);
			...
			// partial data
			criteria = ChartGroupDAO.getInstance().getSession().createCriteria(ChartGroup.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			...
			request.setAttribute("CHARTGROUP",list);
			...
			try {
				ChartGroupDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			if (form.getLong("chartGroupId") == 0) {
				form.setString("chartGroupId",0);
				...
				ChartGroup obj = ChartGroupDAO.getInstance().get(form.getLong("chartGroupId"));
				form.setString("chartGroupId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ChartGroup obj = null;
			if (form.getLong("chartGroupId") == 0) {
				obj = (ChartGroup)ChartGroupDAO.getInstance().getSession().createCriteria(ChartGroup.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new ChartGroup();
					obj.setName(form.getString("name"));
					obj.setDescription(form.getString("description"));
					obj.setGroups(form.getString("groups"));
					obj.setCreateBy(users); 
					obj.setCreateOn(form.getTimestamp("createOn"));
					ChartGroupDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj.setId(form.getLong("chartGroupId"));
				ChartGroupDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ChartGroupDAO.getInstance().delete(form.getLong("chartGroupId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
