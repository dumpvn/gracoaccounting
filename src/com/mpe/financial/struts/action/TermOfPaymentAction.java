...
import com.mpe.financial.struts.form.TermOfPaymentForm;
...
public class TermOfPaymentAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//TermOfPaymentForm uomForm = (TermOfPaymentForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		TermOfPaymentForm form = (TermOfPaymentForm) actionForm;
		...
		try {
			...
			Criteria criteria = TermOfPaymentDAO.getInstance().getSession().createCriteria(TermOfPayment.class);
			...
			// partial data
			criteria = TermOfPaymentDAO.getInstance().getSession().createCriteria(TermOfPayment.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			//criteria.add(Restrictions.eq("Organization.Id", new Long(users.getOrganization().getId())));
			...
			request.setAttribute("TERMOFPAYMENT",list);
			...
			try {
				TermOfPaymentDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			if (form.getLong("termOfPaymentId") == 0) {
				form.setString("termOfPaymentId",0);
				...
				TermOfPayment obj = TermOfPaymentDAO.getInstance().get(form.getLong("termOfPaymentId"));
				form.setString("termOfPaymentId",obj.getId());
				form.setString("name",obj.getName());
				form.setString("code",obj.getCode());
				form.setString("days",obj.getDays());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			TermOfPayment obj = null;
			if (form.getLong("termOfPaymentId") == 0) {
				obj = (TermOfPayment)TermOfPaymentDAO.getInstance().getSession().createCriteria(TermOfPayment.class).add(Restrictions.eq("Code", form.getString("code")))
				//.add(Restrictions.eq("Organization.Id", new Long(users.getOrganization().getId())))
				.uniqueResult();
				if (obj==null) {
					obj = new TermOfPayment();
					obj.setName(form.getString("name"));
					obj.setCode(form.getString("code"));
					obj.setDays(form.getInt("days"));
					obj.setDescription(form.getString("description"));
					obj.setCreateBy(users); 
					obj.setCreateOn(form.getTimestamp("createOn"));
					TermOfPaymentDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
					errors.add("errorDuplicate",new ActionMessage("error.duplicate", form.getString("code")));
					saveErrors(request,errors);
				...
				obj.setId(form.getLong("termOfPaymentId"));
				TermOfPaymentDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDetail(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			request.setAttribute("termOfPayment", termOfPayment);
		}catch(Exception ex) {
			generalError(request,ex);
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			TermOfPaymentDAO.getInstance().delete(form.getLong("termOfPaymentId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
