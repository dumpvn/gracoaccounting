...
import com.mpe.financial.struts.form.JournalTypeForm;
...
public class JournalTypeAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//JournalTypeForm uomForm = (JournalTypeForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		JournalTypeForm form = (JournalTypeForm) actionForm;
		...
		try {
			...
			Criteria criteria = JournalTypeDAO.getInstance().getSession().createCriteria(JournalType.class);
			...
			// partial data
			criteria = JournalTypeDAO.getInstance().getSession().createCriteria(JournalType.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			...
			request.setAttribute("JOURNALTYPE",list);
			...
			try {
				JournalTypeDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			if (form.getLong("journalTypeId") == 0) {
				form.setString("journalTypeId",0);
				...
				JournalType obj = JournalTypeDAO.getInstance().get(form.getLong("journalTypeId"));
				form.setString("journalTypeId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			JournalType obj = null;
			if (form.getLong("journalTypeId") == 0) {
				obj = (JournalType)JournalTypeDAO.getInstance().getSession().createCriteria(JournalType.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new JournalType();
					...
					JournalTypeDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj.setId(form.getLong("journalTypeId"));
				JournalTypeDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			JournalTypeDAO.getInstance().delete(form.getLong("journalTypeId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
