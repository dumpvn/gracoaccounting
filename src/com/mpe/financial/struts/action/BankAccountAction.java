...
import com.mpe.financial.struts.form.BankAccountForm;
...
public class BankAccountAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//BankAccountForm uomForm = (BankAccountForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			Criteria criteria = BankAccountDAO.getInstance().getSession().createCriteria(BankAccount.class);
			...
			// partial data
			criteria = BankAccountDAO.getInstance().getSession().createCriteria(BankAccount.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			criteria.add(Restrictions.eq("Organization.Id", new Long(users.getOrganization().getId())));
			...
			request.setAttribute("BANKACCOUNT",list);
			...
			try {
				BankAccountDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
	    ...
	    List chartOfAccountLst = ChartOfAccountDAO.getInstance().getSession().createCriteria(ChartOfAccount.class)
		  .add(Restrictions.eq("Type", new String("Bank"))).addOrder(Order.asc("Name")).list();
		  request.setAttribute("chartOfAccountLst", chartOfAccountLst);
		  List bankLst = BankDAO.getInstance().getSession().createCriteria(Bank.class).add(Restrictions.eq("Organization.Id", new Long(users.getOrganization().getId()))).addOrder(Order.asc("Name")).list();
		  request.setAttribute("bankLst", bankLst);
		  List customerLst = CustomersDAO.getInstance().getSession().createCriteria(Customers.class).add(Restrictions.eq("Active", Boolean.TRUE)).add(Restrictions.eq("Organization.Id", new Long(users.getOrganization().getId()))).addOrder(Order.asc("Company")).list();
		  request.setAttribute("customerLst", customerLst);
		  List vendorLst = VendorsDAO.getInstance().getSession().createCriteria(Vendors.class).add(Restrictions.eq("Active", Boolean.TRUE)).add(Restrictions.eq("Organization.Id", new Long(users.getOrganization().getId()))).addOrder(Order.asc("Company")).list();
		  ...
		  request.setAttribute("currencyLst", currencyLst);
			if (form.getLong("bankAccountId") == 0) {
				form.setString("bankAccountId",0);
				...
				BankAccount obj = BankAccountDAO.getInstance().get(form.getLong("bankAccountId"));
				form.setString("bankAccountId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (form.getLong("bankAccountId") == 0) {
				obj = (BankAccount)BankAccountDAO.getInstance().getSession().createCriteria(BankAccount.class).add(Restrictions.eq("Number", form.getString("number")))
				...
				if (obj==null) {
					...
					BankAccountDAO.getInstance().save(obj);
				} else {
			    List chartOfAccountLst = ChartOfAccountDAO.getInstance().getSession().createCriteria(ChartOfAccount.class)
				  .add(Restrictions.eq("Type", new String("Bank"))).addOrder(Order.asc("Name")).list();
				  request.setAttribute("chartOfAccountLst", chartOfAccountLst);
				...
				obj.setId(form.getLong("bankAccountId"));
				BankAccountDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			BankAccountDAO.getInstance().delete(form.getLong("bankAccountId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
