...
import com.mpe.financial.struts.form.DegreeForm;
...
public class DegreeAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//DegreeForm uomForm = (DegreeForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		DegreeForm form = (DegreeForm) actionForm;
		...
		try {
			...
			Criteria criteria = DegreeDAO.getInstance().getSession().createCriteria(Degree.class);
			...
			// partial data
			criteria = DegreeDAO.getInstance().getSession().createCriteria(Degree.class);
			if (form.getString("code")!=null && form.getString("code").length()>0)criteria.add(Restrictions.like("Code", "%"+form.getString("code")+"%"));
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			...
			request.setAttribute("DEGREE",list);
			...
			try {
				DegreeDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			if (form.getLong("degreeId") == 0) {
				form.setString("degreeId",0);
				...
				Degree obj = DegreeDAO.getInstance().get(form.getLong("degreeId"));
				form.setString("degreeId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			Degree obj = null;
			if (form.getLong("degreeId") == 0) {
				obj = (Degree)DegreeDAO.getInstance().getSession().createCriteria(Degree.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new Degree();
					...
					DegreeDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj.setId(form.getLong("degreeId"));
				DegreeDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			DegreeDAO.getInstance().delete(form.getLong("degreeId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
