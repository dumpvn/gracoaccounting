...
import com.mpe.financial.struts.form.ItemPriceCategoryForm;
...
public class ItemPriceCategoryAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		...
		//ItemPriceCategoryForm productCategoryForm = (ItemPriceCategoryForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		ItemPriceCategoryForm form = (ItemPriceCategoryForm) actionForm;
		...
		try {
			...
			Criteria criteria = ItemPriceCategoryDAO.getInstance().getSession().createCriteria(ItemPriceCategory.class);
			...
			// partial data
			criteria = ItemPriceCategoryDAO.getInstance().getSession().createCriteria(ItemPriceCategory.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			//if (form.getLong("parentId")>0)criteria.add(Restrictions.eq("Parent.Id", new Long(form.getLong("parentId"))));
			...
			request.setAttribute("ITEMPRICECATEGORY",list);
			...
			try {
				ItemPriceCategoryDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			//List itemCategoryLst = ItemPriceCategoryDAO.getInstance().getSession().createQuery("from ItemPriceCategory as pc where pc.Parent is null").list();
			...
			if (form.getLong("itemPriceCategoryId") == 0) {
				...
				*/
				form.setString("itemPriceCategoryId",0);
				...
				List productIdList = ProductDAO.getInstance().findAll();
				request.setAttribute("productIdList",productIdList);
				List productSelectedIdList = ProductDAO.getInstance().getSession().createCriteria(Product.class).setFetchMode("ProductCategories", FetchMode.JOIN).createCriteria("ProductCategories").add(Expression.eq("Id", new Long(form.getLong("itemPriceCategoryId")))).list();
				request.setAttribute("productSelectedIdList",productSelectedIdList);
				*/
				ItemPriceCategory obj = ItemPriceCategoryDAO.getInstance().get(form.getLong("itemPriceCategoryId"));
				form.setString("itemPriceCategoryId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ItemPriceCategory obj = null;
			if (form.getLong("itemPriceCategoryId") == 0) {
				obj = (ItemPriceCategory)ItemPriceCategoryDAO.getInstance().getSession().createCriteria(ItemPriceCategory.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new ItemPriceCategory();
					...
					//ItemPriceCategory parent = ItemPriceCategoryDAO.getInstance().get(form.getLong("parentId"));
					...
					obj.setProducts(set);
					*/
					ItemPriceCategoryDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj = ItemPriceCategoryDAO.getInstance().get(form.getLong("itemPriceCategoryId"));
				obj.setName(form.getString("name"));
				obj.setNote(form.getString("note"));
				//obj.setActive(form.getString("isActive").length()>0?true:false);
				...
				//ItemPriceCategory parent = ItemPriceCategoryDAO.getInstance().get(form.getLong("parentId"));
				//obj.setParent(parent);
				//obj.setId(form.getLong("itemPriceCategoryId"));
				/*
				String[] productId = form.getCollectionSelect("productSelectedIdList");
				Set set = new LinkedHashSet();
				for (int i=0; i<productId.length; i++) {
				...
				obj.setProducts(set);
				*/
				ItemPriceCategoryDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
					//List itemPriceCategoryLst = ItemPriceCategoryDAO.getInstance().getSession().createQuery("from ItemPriceCategory as pc where pc.Parent is null").list();
					//request.setAttribute("itemPriceCategoryLst",itemPriceCategoryLst);
					/*
					List productIdList = ProductDAO.getInstance().getSession().createCriteria(Product.class).setFetchMode("itemPriceCategory", FetchMode.DEFAULT).add(Expression.eq("itemPriceCategory.id", new Long(form.getLong("itemPriceCategoryId")))).list();
					request.setAttribute("productIdList",productIdList);
					*/
					//List productSelectedIdList = ProductDAO.getInstance().getSession().createQuery("from Product as p where p.itemPriceCategoryId = "+form.getLong("itemPriceCategoryId")).list();
					//request.setAttribute("productSelectedIdList",productSelectedIdList);
				}catch(Exception exx) {
				}
			}catch(Exception exx) {
	...
	private ActionForward performDetail(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			request.setAttribute("itemPriceCategory", itemPriceCategory);
		}catch(Exception ex) {
			generalError(request,ex);
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ItemPriceCategoryDAO.getInstance().delete(form.getLong("itemPriceCategoryId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
