...
import com.mpe.financial.struts.form.CurrencyExchangeRateTypeForm;
...
public class CurrencyExchangeRateTypeAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		throws Exception {
		//CurrencyExchangeRateTypeForm uomForm = (CurrencyExchangeRateTypeForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		CurrencyExchangeRateTypeForm form = (CurrencyExchangeRateTypeForm) actionForm;
		...
		try {
			...
			Criteria criteria = CurrencyExchangeRateTypeDAO.getInstance().getSession().createCriteria(CurrencyExchangeRateType.class);
			...
			// partial data
			criteria = CurrencyExchangeRateTypeDAO.getInstance().getSession().createCriteria(CurrencyExchangeRateType.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			...
			request.setAttribute("CURRENCYEXCHANGERATETYPE",list);
			...
			try {
				CurrencyExchangeRateTypeDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			if (form.getLong("currencyExchangeRateTypeId") == 0) {
				form.setString("currencyExchangeRateTypeId",0);
				...
				CurrencyExchangeRateType obj = CurrencyExchangeRateTypeDAO.getInstance().get(form.getLong("currencyExchangeRateTypeId"));
				form.setString("currencyExchangeRateTypeId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			CurrencyExchangeRateType obj = null;
			if (form.getLong("currencyExchangeRateTypeId") == 0) {
				obj = (CurrencyExchangeRateType)CurrencyExchangeRateTypeDAO.getInstance().getSession().createCriteria(CurrencyExchangeRateType.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new CurrencyExchangeRateType();
					...
					CurrencyExchangeRateTypeDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj.setId(form.getLong("currencyExchangeRateTypeId"));
				CurrencyExchangeRateTypeDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDetail(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			request.setAttribute("currencyExchangeRateType", currencyExchangeRateType);
		}catch(Exception ex) {
			generalError(request,ex);
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			CurrencyExchangeRateTypeDAO.getInstance().delete(form.getLong("currencyExchangeRateTypeId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
