...
import com.mpe.financial.struts.form.StockOpnameTypeForm;
...
public class StockOpnameTypeAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		...
		//StockOpnameTypeForm productCategoryForm = (StockOpnameTypeForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		StockOpnameTypeForm form = (StockOpnameTypeForm) actionForm;
		...
		try {
			...
			Criteria criteria = StockOpnameTypeDAO.getInstance().getSession().createCriteria(StockOpnameType.class);
			...
			// partial data
			criteria = StockOpnameTypeDAO.getInstance().getSession().createCriteria(StockOpnameType.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			//if (form.getLong("parentId")>0)criteria.add(Restrictions.eq("Parent.Id", new Long(form.getLong("parentId"))));
			...
			request.setAttribute("STOCKOPNAMETYPE",list);
			...
			try {
				StockOpnameTypeDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (form.getLong("stockOpnameTypeId") == 0) {
				form.setString("isReceipt","Y");
				form.setString("stockOpnameTypeId",0);
				...
				StockOpnameType obj = StockOpnameTypeDAO.getInstance().get(form.getLong("stockOpnameTypeId"));
				form.setString("stockOpnameTypeId",obj.getId());
				form.setString("name",obj.getName());
				form.setString("code",obj.getCode());
				form.setString("description",obj.getDescription());
				form.setString("isReceipt",obj.isReceipt()==true?"Y":"N");
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			StockOpnameType obj = null;
			if (form.getLong("stockOpnameTypeId") == 0) {
				obj = (StockOpnameType)StockOpnameTypeDAO.getInstance().getSession().createCriteria(StockOpnameType.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new StockOpnameType();
					obj.setName(form.getString("name"));
					obj.setCode(form.getString("code"));
					obj.setDescription(form.getString("description"));
					obj.setReceipt(form.getString("isReceipt").length()>0?true:false);
					...
					StockOpnameTypeDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj = StockOpnameTypeDAO.getInstance().get(form.getLong("stockOpnameTypeId"));
				obj.setName(form.getString("name"));
				obj.setCode(form.getString("code"));
				obj.setDescription(form.getString("description"));
				obj.setReceipt(form.getString("isReceipt").length()>0?true:false);
				...
				StockOpnameTypeDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
	...
	private ActionForward performDetail(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			request.setAttribute("stockOpnameType", stockOpnameType);
		}catch(Exception ex) {
			generalError(request,ex);
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			StockOpnameTypeDAO.getInstance().delete(form.getLong("stockOpnameTypeId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
