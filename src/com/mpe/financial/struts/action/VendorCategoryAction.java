...
import com.mpe.financial.struts.form.VendorsCategoryForm;
...
public class VendorCategoryAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response)
		...
		//VendorsCategoryForm productCategoryForm = (VendorsCategoryForm) form;
	...
	private ActionForward performPartialList(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		VendorsCategoryForm form = (VendorsCategoryForm) actionForm;
		...
		try {
			...
			Criteria criteria = VendorsCategoryDAO.getInstance().getSession().createCriteria(VendorsCategory.class);
			...
			// partial data
			criteria = VendorsCategoryDAO.getInstance().getSession().createCriteria(VendorsCategory.class);
			if (form.getString("name")!=null && form.getString("name").length()>0)criteria.add(Restrictions.like("Name", "%"+form.getString("name")+"%"));
			//if (form.getLong("parentId")>0)criteria.add(Restrictions.eq("Parent.Id", new Long(form.getLong("parentId"))));
			...
			request.setAttribute("VENDORSCATEGORY",list);
			...
			try {
				VendorsCategoryDAO.getInstance().closeSessionForReal();
			}catch(Exception exx) {
			}
		}
		return mapping.findForward("list");
	...
	private ActionForward performForm(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			// relationships
			//List itemCategoryLst = VendorsCategoryDAO.getInstance().getSession().createQuery("from VendorsCategory as pc where pc.Parent is null").list();
			//request.setAttribute("itemCategoryLst",itemCategoryLst);
			if (form.getLong("vendorCategoryId") == 0) {
				form.setString("vendorCategoryId",0);
				...
				List productSelectedIdList = ProductDAO.getInstance().getSession().createCriteria(Product.class).setFetchMode("ProductCategories", FetchMode.JOIN).createCriteria("ProductCategories").add(Expression.eq("Id", new Long(form.getLong("vendorCategoryId")))).list();
				request.setAttribute("productSelectedIdList",productSelectedIdList);
				*/
				VendorsCategory obj = VendorsCategoryDAO.getInstance().get(form.getLong("vendorCategoryId"));
				form.setString("vendorCategoryId",obj.getId());
	...
	private ActionForward performSave(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			VendorsCategory obj = null;
			if (form.getLong("vendorCategoryId") == 0) {
				obj = (VendorsCategory)VendorsCategoryDAO.getInstance().getSession().createCriteria(VendorsCategory.class).add(Restrictions.eq("Name", form.getString("name"))).uniqueResult();
				if (obj==null) {
					obj = new VendorsCategory();
					...
					//VendorsCategory parent = VendorsCategoryDAO.getInstance().get(form.getLong("parentId"));
					...
					obj.setProducts(set);
					*/
					VendorsCategoryDAO.getInstance().save(obj);
				} else {
					ActionMessages errors = new ActionMessages();
				...
				obj = VendorsCategoryDAO.getInstance().get(form.getLong("vendorCategoryId"));
				obj.setName(form.getString("name"));
				obj.setDescription(form.getString("description"));
				//obj.setActive(form.getString("isActive").length()>0?true:false);
				...
				//VendorsCategory parent = VendorsCategoryDAO.getInstance().get(form.getLong("parentId"));
				//obj.setParent(parent);
				//obj.setId(form.getLong("vendorCategoryId"));
				/*
				String[] productId = form.getCollectionSelect("productSelectedIdList");
				Set set = new LinkedHashSet();
				for (int i=0; i<productId.length; i++) {
				...
				obj.setProducts(set);
				*/
				VendorsCategoryDAO.getInstance().update(obj);
			}
		}catch(Exception ex) {
			try {
				try {
					//List vendorCategoryLst = VendorsCategoryDAO.getInstance().getSession().createQuery("from VendorsCategory as pc where pc.Parent is null").list();
					//request.setAttribute("vendorCategoryLst",vendorCategoryLst);
					/*
					List productIdList = ProductDAO.getInstance().getSession().createCriteria(Product.class).setFetchMode("vendorCategory", FetchMode.DEFAULT).add(Expression.eq("vendorCategory.id", new Long(form.getLong("vendorCategoryId")))).list();
					request.setAttribute("productIdList",productIdList);
					*/
					//List productSelectedIdList = ProductDAO.getInstance().getSession().createQuery("from Product as p where p.vendorCategoryId = "+form.getLong("vendorCategoryId")).list();
					//request.setAttribute("productSelectedIdList",productSelectedIdList);
				}catch(Exception exx) {
				}
			}catch(Exception exx) {
	...
	private ActionForward performDetail(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		//HttpSession httpSession = request.getSession();
		try {
			VendorsCategory vendorCategory = VendorsCategoryDAO.getInstance().get(form.getLong("vendorCategoryId"));
			request.setAttribute("vendorCategory", vendorCategory);
		}catch(Exception ex) {
			generalError(request,ex);
	...
	private ActionForward performDelete(ActionMapping mapping, ActionForm actionForm, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			VendorsCategoryDAO.getInstance().delete(form.getLong("vendorCategoryId"));
		}catch(Exception ex) {
			try {
			}catch(Exception exx) {
			}
