        ...
        <TR>
	      ...
	      <table width="97%" border="0" cellspacing="2" cellpadding="0">
	      	<tr valign="top"><td><html:errors/></td></tr>
    			...
    			<html:form action="/itemReport/penjualanDetail">
                ...
                <tr> 
                  <td><bean:message key="page.color.title"/></td><td>:</td>
                  <td>
                  	<html:select property="itemColorId">
											<html:option value=""><bean:message key="page.selectBellow.title"/></html:option>
											<html:options collection="itemColorLst" property="id" labelProperty="name" />
                  	</html:select>
                  </td>
                </tr>
    						<tr>
            ...
            <td align="center"> 
              <table border="0" cellspacing=1 cellpadding=2 bgcolor="#cccccc" width="100%">
                <tr align="center"> 
                  ...
                  <bean:size id="c" name="itemSizeLst"/>
                	<bean:define id="d" value="0"/>
                	<%d = String.valueOf(c);%>
                	<%int e=0; e = Integer.parseInt(d)+1;%>
                  <td class=titleField colspan="<%=e%>">Keluar</td>
                  <td class=titleField colspan="<%=e%>">Saldo</td>
                  <td class=titleField rowspan="2"><bean:message key="page.note.title"/></td>
                </tr>
                <tr>
...
%>
                <logic:iterate id="penjualanDetail" name="penjualanDetailLst" type="com.mpe.financial.model.ItemPosReportPenjualanDetail">
								<% 
									sequence++;
								...
								<tr bgcolor="<% out.print(bgcolor); %>">
									<td align="right"><%=++i%>.</td>
									<td><bean:write name="penjualanDetail" property="formatedDate" scope="page"/></td>
									<td><bean:write name="penjualanDetail" property="number" scope="page"/></td>
                  <logic:iterate id="itemSize" name="itemSizeLst" type="com.mpe.financial.model.ItemSize">
                  <td align="right"></td>
                  </logic:iterate>
                  <td></td>
                  ...
                  <td></td>
									<td><bean:write name="penjualanDetail" property="note" scope="page"/></td>
                </tr>
                </logic:iterate>
              </table>
            </td>
    			...
    			<tr>
    				<td align="center">
    					<table width="10%">
    						<tr>
    							<td align="center">
    								<input class="Button" type="button" name="create" onclick="window.open('<html:rewrite page="/itemReport/penjualanDetailPdf.do"/>?locationId=<bean:write name="itemForm" property="locationId"/>&itemMerkId=<bean:write name="itemForm" property="itemMerkId"/>&itemTypeId=<bean:write name="itemForm" property="itemTypeId"/>&itemColorId=<bean:write name="itemForm" property="itemColorId"/>&fromDate=<bean:write name="itemForm" property="fromDate"/>&toDate=<bean:write name="itemForm" property="toDate"/>','gg','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=yes,width=800,height=600');" value="<bean:message key="page.print.link"/>">
    							</td>
    						</tr>
    					</table>
    				</td>
